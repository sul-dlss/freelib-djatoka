...
public class OpenURLJP2KService implements Service, FormatConstants {
    ...
    private static final String PROPS_KEY_CACHE_ENABLED =
            "OpenURLJP2KService.cacheEnabled";
    ...
    private static final String PROPS_KEY_CACHE_SIZE =
            "OpenURLJP2KService.cacheSize";

    private static final String PROP_KEY_CACHE_MAX_PIXELS =
            "OpenURLJP2KService.cacheImageMaxPixels";
        ...
        try {
            if (!init) {
                ...
                if (props.getProperty(PROPS_KEY_TRANSFORM) != null) {
                    ...
                    transform =
                            (ITransformPlugIn) Class.forName(transClass)
                                    .newInstance();
    ...
    public OpenURLResponse resolve(ServiceType serviceType,
        ...
        if (params.getRegion() != null && params.getRegion().contains("-")) {
            try {
                bytes =
                        ("Negative Region Arguments are not supported.")
                                .getBytes("UTF-8");
        ...
        OpenURLResponse response =
                new OpenURLResponse(status, responseFormat, bytes, header_map);
    ...
    private static String getTileHash(String id, DjatokaDecodeParam params)
            throws Exception {
                ...
                if (tmp.getClass().getSimpleName()
                    ...
                    if (kev.getFieldMap().size() > 0) {
                        ...
                        if (kev.getFieldMap().containsKey("svc.level") &&
                            ...
                            map.put("level", ((String[]) kev.getFieldMap().get(
                                    "svc.level"))[0]);
                        }
                        ...
                        if (kev.getFieldMap().containsKey("svc.scale") &&
                            ...
                            map.put("scale", ((String[]) kev.getFieldMap().get(
                                    "svc.scale"))[0]);
                        }
