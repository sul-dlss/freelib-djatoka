...
public class GIFWriter implements IWriter {
    ...
    class GifEncoder {
        ...
        int sample = 2; // default sample interval for quantizer distance should
                        // be small for small icons

        byte[] gct = null; // Global color table

        boolean GCTextracted = false; // Set if global color table extracted
                                      // from rgb image

        boolean GCTloadedExternal = false; // Set if global color table loaded
                                           // directly from external image
        ...
        boolean GCTsetTransparent = false; // If true then Color table
                                           // transparency index is set

        boolean GCToverideIndex = false; // If true Transparent index is set to
                                         // index with closest colors

        boolean GCToverideColor = false; // if true Color at Transparent index
                                         // is set to GCTred, GCTgrn GCTbl
        ...
        /*
         * Get Options because options box has been checked Some of the code
         * being set setTransparent(Color.black); Dispose = 0; setDelay(500); //
         * time per frame in milliseconds gctused = false; // Set to true to use
         * Global color table GCTextracted = false; // Set if global color table
         * extracted from rgb image GCTloadedExternal = false; // Set if global
         * color table loaded directly from external image GCTextracted = false;
         * // Set if global color table extracted from rgb image GCTred = 0;
         * //Transparent Color GCTgrn = 0; // green GCTbl = 0; // blue GCTcindex
         * = 0; //index into color table autotransparent = false; // Set True if
         * transparency index coming from image 8 bit only GCTsetTransparent =
         * true; //If true then Color table transparency index is set
         * GCToverideIndex = false; //If true Transparent index is set to index
         * with closest colors GCToverideColor = false; //if true Color at
         * Transparent index is set to GCTred, GCTgrn GCTbl
         */
        ...
        public boolean finish() {
            ...
            GCTextracted = false; // Set if global color table extracted from
                                  // rgb image
            GCTloadedExternal = false; // Set if global color table loaded
                                       // directly from external image
            transIndex = 0;
            transparent = false;
        ...
        /**
         * Sets the GIF frame size. The default size is the size of the first
         * frame added if this method is not invoked.
         * 
        ...
        /**
         * Sets the transparent color for the last added frame and any
         * subsequent frames. Since all colors are subject to modification in
         * the quantization process, the color in the final palette for each
         * frame closest to the given color becomes the transparent color for
         * that frame. May be set to null to indicate no transparent color.
         * 
        ...
        protected void writeLSDgct() throws IOException {
            ...
            writeShort(height);
            // packed fields:
            // 1 : global color table flag = 0 (nn
        ...
        protected void writeLSD() throws IOException {
            ...
            writeShort(height);
            // packed fields:
            // 1 : global color table flag = 0 (none)
            // 2-4 : color resolution = 7
            // 5 : gct sort flag = 0
            // 6-8 : gct size = 0
        ...
        protected void writeNetscapeExt() throws IOException {
            ...
            writeShort(repeat); // loop count (extra iterations, 0=repeat
                                // forever)
            out.write(0); // block terminator
        }
