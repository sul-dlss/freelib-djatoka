...
public class ImageJReader implements IReader {
    ...
    public BufferedImage open(final InputStream input) throws FormatIOException {
        ...
        final ImagePlus imp = o.openTiff(input, "tif");
        ...
        if (imp == null) {
            ...
            final File path = IOUtils.createTempImage(input);
     ...
     * @throws FormatIOException
     */
    private BufferedImage open(final ImagePlus imp) throws FormatIOException {
        if (imp == null) {
        ...
        imp.flush(); // Only good for VRAM
        // imp = null;
        ip = null;

        final BufferedImage bImg = new BufferedImage(width, height, BufferedImage.TYPE_3BYTE_BGR);
