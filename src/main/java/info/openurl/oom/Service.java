...
/**
 * If you think of a service in terms of pure business logic, this class provides a simple bridge providing access to
 * it from the web. To use another analogy, if the business logic is a coat, and OpenURL is a coatrack, this class
 * represents a loop of cloth sewn into the collar of the coat.
 * <p />
 * Installing new Services can be as easy as dropping them on the classpath, if your Transport is smart enough to
 * deduce the package and classname from information in the HttpServletRequest. If the Transport can't deduce
 * everything it needs from the HTTP request, it could use a config file to locate and configure Service classes
 * instead.
 *
...
public interface Service {
    ...
    /**
     * This method is responsible for pulling what (Referent), why (ServiceType), who (Requester), etc. information
     * out of the ContextObject and using it to call any Java classes and methods needed to produce a result. Having
     * obtained a result of some sort from the business logic, this method is then responsible for transforming it
     * into an OpenURLResponse that acts as a proxy for HttpServletResponse.
     *
